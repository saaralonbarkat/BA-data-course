...
First, we need to call/open the `tidyverse` library

```{r}
library(tidyverse)
```

Once we called these packages, we can use the tidyverse functions. 


Next, let's save the gapminder as a dataframe in the working environment. Assign the name gapminder and use the read_csv function

```{r}
gapminder <- read_csv("https://raw.githubusercontent.com/saaralonbarkat/BA-data-course/refs/heads/main/data/gapminder/gapminder_modern_1952_2022.csv")
...
We will create a subset of the data, only data from 2007. We will use `filter`. Assign the name `gapminder_2022`.

```{r}
gapminder_2022 <- gapminder %>% 
  filter(year==2022)
```
...
We can arrange by frequency, using `arrange`
```{r}
gapminder_2022 %>% 
  count(continent) %>% 
  arrange(n %>% desc())
...
Now, try yourself to explore another categorical variable - regime. Count the number of countries per each regime type:
...
For self practice - try to do the same with GDP per capita by regime type
